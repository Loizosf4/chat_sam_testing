...
test("transport invokes native-style fetch with the browser global receiver",async()=>{let receiver;const fetchImpl=function(){receiver=this;return ok(ack)};const client=new BlenderSyncClient({fetchImpl});client.scene=scene();await client.select(object);assert.equal(receiver,globalThis)});

test("scene synchronization uses the same-origin proxy and sends stable IDs",async()=>{const calls=[];const timer={setInterval:()=>1,clearInterval:()=>{}};const client=new BlenderSyncClient({timer,fetchImpl:async(url,options)=>{calls.push({url,options});return url.includes("/events")?ok({latest_sequence:0,events:[]}):ok(ack)}});await client.connect(scene());assert.equal(calls[0].url,"/api/blender-sync/command");assert.equal(JSON.parse(calls[0].options.body).type,"handshake");const sync=JSON.parse(calls[1].options.body);assert.equal(sync.type,"scene_sync");assert.equal(sync.object_id,null);assert.equal(sync.payload.scene_package.semantic_objects[0].object_id,object.object_id);assert.equal(sync.revision,7);client.disconnect()});
