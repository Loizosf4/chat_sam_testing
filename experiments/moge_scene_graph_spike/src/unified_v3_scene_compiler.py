...
def _write_outputs(output:Path,source:Image.Image,metadata:dict[str,Any],masks:dict[str,np.ndarray],scene:dict[str,Any],collisions:list[dict[str,Any]],guard:CleanReadGuard,plane_diag:dict[str,Any],per_object:list[tuple],handoff:Path,moge_meta:dict[str,Any]):
    ...
    by_label={o["semantic_label"]:o for o in objects};desktop=by_label["desktop_box"];desk=by_label["desk"];chair=by_label["desk_chair"];desktop_bottom=float(desktop["transform"]["center"][2]-desktop["transform"]["dimensions"][2]/2);desk_top=float(scene["support_graph"]["top_surfaces"][desk["object_id"]]["z"])
    quality_gates={"exactly_20_semantic_cubes":len(objects)==20,"normal_first_v3_for_all":all(o["orientation_method"]=="normal_first_v3_universal" for o in objects),"desktop_box_supported_by_desk":desktop["support_target"]==desk["object_id"] and desktop["support_type"]=="object","desktop_box_not_below_desk":desktop_bottom>=desk_top-1e-6,"desktop_box_placement_valid":desktop["placement_classification"] in ("placement_high_confidence","placement_with_occlusion") and not desktop["placement_hard_gates"],"chair_occlusion_aware_visible_proxy":chair["placement_classification"]=="placement_with_occlusion" and chair["occlusion"]["partially_occluded"] and "visible_surface_proxy" in chair["occlusion"]["hidden_geometry_policy"],"no_invalid_placements":placement_counts["placement_invalid"]==0,"joint_room_fit_passed":bool(room_fit["passed"]),"zero_forbidden_input_reads":not violations and not guard.denied_log}
    placement_report={"schema_version":"1.0","counts":placement_counts,"quality_gates":quality_gates,"objects":[{"object_id":o["object_id"],"semantic_label":o["semantic_label"],"support_type":o["support_type"],"support_target":o["support_target"],"placement_classification":o["placement_classification"],"hard_gates":o["placement_hard_gates"],"bbox_iou":o["validation_metrics"]["bbox_iou"],"visible_hull_iou":o["validation_metrics"]["visible_hull_iou"],"centroid_error_pixels":o["validation_metrics"]["centroid_error_pixels"],"occlusion":o["occlusion"]} for o in objects]};compilation={"schema_version":"1.0","passed":all(quality_gates.values()),"quality_gates":quality_gates,"object_count":len(objects),"universal_v3_invocations":sum(o["orientation_method"]=="normal_first_v3_universal" for o in objects),"confidence_counts":counts,"placement_counts":placement_counts,"clean_output_hash_pending":True}
